...
class Agent(object):
    ...
    def updateFitness4(self):
        currentPos = self.robot.get_centroid()
        self.fitness += ( 1 - abs(self.rotationValue/maxRotationSpeed) ) * math.sqrt(abs(currentPos[0]**2-self.previousPos[0]**2)) + math.sqrt(abs(currentPos[1]**2-self.previousPos[1]**2)) + self.bonus # a chaque pas de temps, ajoute la distance parcourue depuis t-1, avec une pénalité si rotation
        self.previousPos = currentPos
    ...
    def stepController(self):
        ...
        t = math.tanh( sensorMinus80 * self.params[0] + sensorMinus40 * self.params[1] + sensorMinus20 * self.params[2] + sensorPlus20 * self.params[3] + sensorPlus40 * self.params[4] + sensorPlus80 * self.params[5] + self.params[6] )
        if t < 0:
            t = 0
        translation =  t
        ...
        coord = agents[0].getRobot().get_centroid()
        if occupancyGrid[int(coord[0])/16][int(coord[1])/16] == 'X':
            self.bonus -= 4
        else:
            self.bonus += 10
            occupancyGrid[int(coord[0])/16][int(coord[1])/16] = 'X'
